...
const QuizResultPage = () => {
  ...
  const detailedResults = data.result || [];
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl shadow-xl shadow-slate-200/50 p-8 mb-8">
        <div className="flex flex-col items-center justify-center text-center space-y-6">
          <div
            className={`inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-linear-to-br ${getScoreColor(score)} shadow-lg shadow-emerald-500/20`}
          >
      ...
      <div className="space-y-6">
        ...
        {detailedResults.map((result, index) => {
          ...
          // Enhanced logic to handle '0' (Zero) or 'O' (Letter O) prefixes
          let correctAnswerIndex = -1;

          // Check for index based answer (e.g. "01", "1")
          if (
            result.correctAnswer.startsWith("0") &&
            !isNaN(parseInt(result.correctAnswer[1]))
          ) {
            correctAnswerIndex =
              parseInt(result.correctAnswer.substring(1)) - 1;
          }
          // Check for "O1:", "O2:" format matching your dump
          else if (
            result.correctAnswer.startsWith("O") &&
            !isNaN(parseInt(result.correctAnswer[1]))
          ) {
            correctAnswerIndex =
              parseInt(result.correctAnswer.substring(1)) - 1;
          ...
          // Fallback to exact string match
            ...
            // If still -1, try finding the option that is contained within the correctAnswer string
            if (correctAnswerIndex === -1) {
              correctAnswerIndex = result.options.findIndex((opt) =>
                result.correctAnswer.includes(opt)
              );
            }
          }
            ...
            >
              ...
              <div className="flex items-start justify-between gap-4 mb-3">
                <div className="flex-1">
                  {/* Updated Question Span */}
                  <span className="inline-flex items-center px-3 py-1 rounded-lg text-xs font-medium bg-slate-50 text-slate-700 border border-slate-200 mb-3">
                    Question {index + 1}
                  </span>
                  ...
                  return (
                    <div
                      ...
                      className={`relative px-4 py-3 rounded-lg border-2 transition-all duration-200 ${
                        isCorrectOption
                          ? "bg-emerald-50 border-emerald-300 shadow-lg shadow-emerald-500/25"
                          : isWrongAnswer
                    ...
                    >
                      <div className="flex items-center justify-between gap-3">
                        ...
                        {isWrongAnswer && (
                          <span className="inline-flex items-center gap-1 px-2 py-1 bg-rose-100 border border-rose-200 rounded-lg text-xs font-semibold text-rose-700">
              ...
              {/* {result.explanation && (
                <div className="p-4 bg-linear-to-br from-slate-50 to-slate-100/50 border border-slate-200 rounded-xl">
                  <div className="flex items-start gap-3">
                    <BookOpen
                      className="w-4 h-4 text-slate-600 mt-0.5 border"
                      strokeWidth={2}
                    />
                    <div>
              ...
              {result.explanation && (
                <div className="p-4 bg-linear-to-br from-slate-50 to-slate-100/50 border border-slate-200 rounded-xl">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="shrink-0 flex items-center justify-center w-7 h-7 rounded-md bg-slate-200/70">
                      <BookOpen
      ...
      <div className="mt-8 flex justify-center">
        <Link to={`/documents/${quiz.document?._id}`}>
          <button className="group relative px-8 h-12 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 active:scale-95 overflow-hidden">
