...
const QuizTakePage = () => {
  ...
  const handleOptionChange = (mcqId, optionIndex) => {
  ...
  const handleSubmitQuiz = async () => {
    ...
    try {
      const formattedAnswers = Object.keys(selectedAnswers).map((mcqId) => {
        const mcq = quiz.mcqs.find((mcq) => mcq._id === mcqId);
        const mcqIndex = quiz.mcqs.findIndex((mcq) => mcq._id === mcqId);
        const optionIndex = selectedAnswers[mcqId];
        const selectedAnswer = mcq.options[optionIndex];

        return { mcqIndex, selectedAnswer };
      });

      const { data } = await quizService.submitQuiz(quizId, formattedAnswers);
      // console.log(data);
  ...
  if (!quiz || quiz.mcqs.length === 0) {
    return (
      <div className="text-center">
        <p className="text-slate-600 text-lg">Quiz not found or has no mcqs.</p>
      </div>
    );
  }

  const currentMCQ = quiz.mcqs[currentMCQIndex];
  const isAnswered = selectedAnswers.hasOwnProperty(currentMCQ._id);
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <div className="relative h-2 bg-slate-100 rounded-full overflow-hidden">
          <div
            ...
            style={{
              width: `${((currentMCQIndex + 1) / quiz.mcqs.length) * 100}%`,
            }}
          />
        </div>
      </div>
      ...
      <div className="bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl shadow-xl shadow-slate-200/50 p-6 mb-8">
        ...
        <div className="space-y-3">
          {currentMCQ.options.map((option, index) => {
            const constIsSelected = selectedAnswers[currentMCQ._id] === index;
            return (
              <label
                key={index}
              ...
              >
                <input
                  type="radio"
                  name={`question-${currentMCQ._id}`}
                  value={index}
                  checked={constIsSelected}
                  onChange={() => handleOptionChange(currentMCQ._id, index)}
                  className="sr-only"
                />
      ...
      <div className="flex items-center justify-between gap-4">
        <Button
          ...
          disabled={currentMCQIndex === 0 || submitting}
          variant="secondary"
        >
        ...
        {currentMCQIndex === quiz.mcqs.length - 1 ? (
          <Button
          ...
          <Button onClick={handleNextMCQ} disabled={submitting}>
            Next
      ...
      <div className="mt-8 flex items-center justify-center gap-2 flex-wrap">
        {quiz.mcqs.map((_, index) => {
          ...
          const isCurrent = index === currentMCQIndex;
          ...
          return (
            <button
              ...
              className={`w-8 h-8 rounded-lg font-semibold text-xs transition-all duration-200 ${
                isCurrent
                  ? "bg-linear-to-r from-emerald-500 to-teal-500 text-white shadow-lg shadow-emerald-500/30 scale-110"
                  : isAnsweredQuestion
