...
const FlashcardManager = ({ documentId }) => {
  ...
  const handleNextCard = () => {
    if (selectedSet) {
      // Review current before moving
      handleReview(currentCardIndex);
      setCurrentCardIndex((prevIndex) =>
        prevIndex + 1 < selectedSet.cards.length ? prevIndex + 1 : prevIndex
      );
    }
  };
  ...
  const handleReview = async (index, cardOverride = null) => {
    const currentCard = cardOverride || selectedSet?.cards[index];
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-center gap-6">
        <button
          ...
          className="group flex items-center gap-2 px-5 h-11 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium text-sm rounded-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          <ChevronLeft
            ...
            <div
              ...
              onClick={() => {
                ...
                if (set.cards && set.cards.length > 0) {
                  handleReview(0, set.cards[0]);
                }
              }}
              className="group relative bg-white/80 backdrop-blur-xl border border-slate-200 hover:border-emerald-300 rounded-2xl p-6 cursor-pointer transition-all duration-200 hover:shadow-lg hover:shadow-emerald-500/10 active:scale-[0.98]"
            >
              {/* Delete Button */}
              <button
              ...
              <div className="space-y-4">
                <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-linear-to-br from-emerald-100 to-teal-100 text-emerald-600">
  ...
  return (
    <>
      <div className="bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-lg shadow-slate-200/50 p-6 relative overflow-hidden">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            ...
            <button
              ...
              disabled={deleting}
              className="px-5 h-11 bg-linear-to-r from-rose-500 to-red-500 hover:from-rose-600 hover:to-red-600 text-white font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-rose-500/25 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {deleting ? (
                <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
              ) : (
...
export default FlashcardManager; // DocumentFlashcardSets
